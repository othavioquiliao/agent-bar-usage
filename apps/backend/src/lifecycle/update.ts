...
export async function runUpdate(deps?: UpdateDependencies): Promise<void> {
  ...
  const logResult = await runGit(
    ["log", "--oneline", "HEAD..origin/master", "-15"],
    run,
  );
  ...
  if (!pullResult.ok) {
    ...
    p.log.warn(
      "If you have local commits, try: git rebase origin/master",
    );
    p.outro("Update failed");
    return;
  }
  ...
  try {
    ...
    p.log.warn(
      "systemctl commands failed -- restart the service manually if needed",
    );
  }
  ...
  p.log.success(
    `Updated: ${currentCommit.output} -> ${newCommit.output}`,
  );
