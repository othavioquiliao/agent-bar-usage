...
import { APP_NAME, ENV_VARS_TO_CAPTURE, EXT_ITEMS, GNOME_EXT_UUID, getInstallPaths, REPO_ROOT } from './paths.js';
...
export async function runSetup(deps?: SetupDependencies): Promise<void> {
  ...
  if (missing.length > 0) {
    const lines = missing.map((dep) => `  - ${dep.name}: ${dep.installHint}`);
  ...
  try {
    ...
    const wrapperContent = ['#!/usr/bin/env bash', 'set -euo pipefail', `exec bun "${cliEntry}" "$@"`, ''].join('\n');
    ...
    try {
      ...
      p.log.warn('systemctl commands failed -- service may need manual start');
    }
    ...
    try {
      ...
      p.log.warn('Could not enable GNOME extension automatically. Enable it manually in Extensions app.');
    }
    ...
    if (!pathDirs.some((d) => resolvePath(d) === resolvePath(localBin))) {
      p.log.warn('~/.local/bin is not in your PATH. Add: export PATH="$HOME/.local/bin:$PATH"');
    }
