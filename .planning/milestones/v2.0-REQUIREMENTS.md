# Requirements Archive: v2.0 Refactor & Polish
...
- [x] **CLI-01**: CLI uses manual argument parsing without Commander (switch/case + suggestCommand with Levenshtein)
- [x] **CLI-02**: Schema validation uses inline type guards and assertion functions without Zod
- [x] **CLI-03**: `--help` displays formatted help output with box-drawing characters
...
- [x] **PROV-01**: Each provider is a self-contained module with zero cross-provider imports
- [x] **PROV-02**: Provider interface follows minimal contract: `{ id, name, cacheKey, isAvailable(), getQuota() }`
- [x] **PROV-03**: User can select which providers appear in GNOME topbar via `agent-bar providers` CLI command
- [x] **PROV-04**: Existing SVG/PNG icon assets are properly integrated and displayed in the GNOME extension

### Data & Formatting

- [x] **DATA-01**: Backend auto-refreshes provider data periodically via configurable setInterval (default 150s)
- [x] **DATA-02**: File-based cache with TTL and fetch deduplication stored in XDG_CACHE_HOME
- [x] **DATA-03**: Date/time formatting is locale-aware using Intl.RelativeTimeFormat and Intl.DateTimeFormat
- [x] **DATA-04**: Settings are versioned with migration logic and atomic writes (temp file + rename)

### Terminal UI

- [x] **TUI-01**: Interactive menu via @clack/prompts with actions: List All, Configure Providers, Provider Login, Doctor
- [x] **TUI-02**: Terminal quota display with Unicode progress bars and One Dark Pro color palette
- [x] **TUI-03**: Doctor command outputs checks with @clack/prompts spinners and colored pass/fail indicators
- [x] **TUI-04**: Provider login TUI guides user through Claude, Codex, and Copilot auth flows

### Code Quality

- [x] **QUAL-01**: Biome replaces ESLint/Prettier for linting and formatting
- [x] **QUAL-02**: Obvious bugs and code smells are fixed throughout the codebase
- [x] **QUAL-03**: Real GitHub OAuth App registered and DEFAULT_CLIENT_ID placeholder replaced
...
| TUI-01 | Phase 12 | Complete |
| TUI-02 | Phase 12 | Complete |
| TUI-03 | Phase 12 | Complete |
...
*Last updated: 2026-03-29 after Phase 12 verification*
